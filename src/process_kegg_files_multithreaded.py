...
def main(): # pragma: no cover
    ...
    for org_code in selected_organisms:
        ...
        # generate all gene names
        gene_names = [ gene_name for gene_name, ko_id, nt_seq, aa_seq in org_code_to_gene_and_ko[org_code] ]
        ...
        #gene_start_end_strand_infos = [read_gene_start_and_end_positions(gene_name) for gene_name in gene_names]

        # build a dictionary, indexed by gene_names, and mapping to start, end pos, and strand info
        gene_name_to_start_end_strand_info = {}
        for gene_name, start_end_strand_info in list( zip(gene_names, gene_start_end_strand_infos) ):
            gene_name_to_start_end_strand_info[gene_name] = start_end_strand_info

        # iterate through all gene names, and generate mapping records
        for gene_name, ko_id, nt_seq, aa_seq in tqdm(org_code_to_gene_and_ko[org_code]):
            start_pos, end_pos, strand = gene_name_to_start_end_strand_info[gene_name]
